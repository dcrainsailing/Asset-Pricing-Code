...
class ETF:
    ...
    def auto_regression(self):
        ...
        data_lag.rename(columns={'ETF_Daily_return': 'ETF_Daily_return_lag'}, inplace=True)
        data['ETF_Daily_return_lag'] = data_lag['ETF_Daily_return_lag']
        data = data.dropna()
        regress_result = stats.linregress(data.iloc[:, 1], data.iloc[:, 0])
...
def multi_annualized_return(start_date, end_date):
    etf = ETF('SPY',start_date,end_date)
...
def multi_covariance(frequence = 'Daily',start_date = '2010-01-01',end_date = '2019-11-18'):
    etf = ETF('SPY',start_date, end_date)
